    ...
    <div className="min-h-screen bg-slate-50 p-8 text-slate-900 font-sans">
      <header className="mb-10 flex items-center justify-between border-b pb-4">
        ...
        <button
          ...
          disabled={loading}
          className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
          {loading ? (
            <>
            ...
            "Simulate AWS Deployment (s3:CreateBucket)"
          )}
        </button>
      </header>
      ...
      {!auditResult && !loading && (
        <div className="flex flex-col items-center justify-center py-20 text-slate-400 border-2 border-dashed rounded-xl bg-white">
          ...
          <p className="text-sm mt-2">Click the simulate button above to trigger the LangGraph Swarm.</p>
        </div>
      )}
      ...
      {auditResult && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <span className="font-medium">{auditResult.resource_id}</span>
                </div>
                <div>
                  <span className="text-sm text-slate-500 block">Compliance Status</span>
                  {auditResult.compliance_passed ? (
                    <span className="inline-flex items-center gap-1 text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md font-medium text-sm mt-1">
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"></path></svg>
                      Compliant
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-red-600 bg-red-50 px-2 py-1 rounded-md font-medium text-sm mt-1">
                ...
                {auditResult.ticket_created && (
                  <div>
                    ...
                    <span className="text-indigo-600 font-medium bg-indigo-50 px-2 py-1 rounded-md inline-block mt-1">
                      {auditResult.ticket_created}
                    </span>
                  </div>
                )}
              ...
              <ol className="relative border-l border-slate-200 ml-3 space-y-6">
                {auditResult.agent_logs.map((log: string, idx: number) => (
                  <li key={idx} className="ml-6">
                    <span className="absolute flex items-center justify-center w-6 h-6 bg-indigo-100 rounded-full -left-3 ring-4 ring-white text-indigo-600 text-xs font-bold">
                      {idx + 1}
                    </span>
                    <p className="text-sm text-slate-700 bg-slate-50 p-3 rounded-md border">{log}</p>
                  </li>
                ))}
              </ol>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-slate-900 rounded-xl shadow-sm overflow-hidden h-full flex flex-col">
              <div className="bg-slate-800 px-4 py-3 flex items-center justify-between border-b border-slate-700">
                ...
                <span className="text-xs bg-slate-700 text-slate-300 px-2 py-1 rounded">Terraform</span>
              </div>
              <div className="p-4 flex-grow overflow-auto">
                {auditResult.remediation_code ? (
                  ...
                  <div className="h-full flex items-center justify-center text-slate-500">
                    No remediation required. Infrastructure is compliant.
                  </div>
                )}
              </div>
            </div>
